...
export default class RemoveKey extends Command {
  ...
  async run() {
    ...
    // Prompt if needed
    if (!args.privateKey) {
      args.privateKey = await CliUx.ux.prompt('Enter private key to delete (starts with PVT_K1)', { type: 'mask' })
    }

    await passwordManager.removePrivateKey(args.privateKey)

    CliUx.ux.log(`Successfully removed private key for public key ${Key.PrivateKey.fromString(args.privateKey).getPublicKey()}`)
  }
